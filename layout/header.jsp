...
<link rel="stylesheet" type="text/css" href="css/style-favorite.css">
...
<!-- End of Script -->
<title>JYP SHOP</title>
</head>

<body>
	
	<a id="header-banner-a" href="<%=request.getContextPath()%>/"><img
		src="images/header/bannerImage.jpg" id="header-banner" /></a>
	<header>
		<div id="header-first">
			<div id="header-logo">
				<a href="<%=request.getContextPath()%>"><img
					src="images/header/mainLogo.png"  id="header-logo-img" /></a>
			</div>
			...
			<c:choose>
				<c:when test="${sessionScope.principal != null}">
					...
					<!-- sessionScope.principal.auth 가 null임-->
		...
		<div id="header-second">
			<div class="btn-group header-second-btn-box">
				<c:choose>
					<c:when
						test="${pageContext.request.requestURI eq 'main.jsp' }">
				...
				<c:choose>
					<c:when test="${pageContext.request.requestURI eq '/shop2/product/rank.jsp' }">
						<button type="button" id="header-sec-rank" class="btn btn-basic header-second-btn-group border-btm-red" onclick="location.href='/shop2/product?cmd=rank';">랭킹</button>
					</c:when>
					<c:otherwise>
