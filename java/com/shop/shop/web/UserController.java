...
public class UserController extends HttpServlet {
	...
	protected void doProcess(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		if (cmd.equals("joinForm")) {
			...
			if (result == 1) {
				User userEntity = userService.로그인(joinUser.getUsername(), joinUser.getPassword(),joinUser.getAuth());
				HttpSession session = request.getSession();
				session.setAttribute("principal", userEntity);
				dis = request.getRequestDispatcher("/index.jsp");
				dis.forward(request, response);
			...
			User userEntity = userService.로그인(request.getParameter("username"), SHA256.toSHA256(request.getParameter("password")),request.getParameter("auth"));
			if (userEntity == null) {
